...
class adalineSDG(object):
    ...
    def _shuffle(self,X, y):
        seq = np.random.permutation(lean(y))
        return X[seq], y[seq]
    ...
    def fit (self, X, y):
        ...
        for i in range (self.n_inter):
            if self._shuffle:
                ...
                cost=[]
                for xi, tarjet in zip(X,y):
                    cost .append(self._update_weights(xi, tarjet))
            ...
            def partial_fit(self,X,y):
                if not self.w_inicialized:
                    self._inicialized_weights(X.shape[1])
                if y.ravel().shape[0] > 1:
                    for xi, tarjet in zip (X,y):
                        self._update_weights(xi, tarjet)
                    else:
